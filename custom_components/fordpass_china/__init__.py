...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    dlt_token = config.get(CONF_DLT_TOKEN)
    auth_type = config.get(CONF_AUTH_TYPE, "dlt" if dlt_token else "refresh_token")
    ...
    # 按认证模式建立可用令牌
    if auth_type == "dlt" or (dlt_token and not refresh_token):
        if not await fordpass.check_token():
            _LOGGER.error("[%s] DLT 访问令牌为空，请重新配置", entry.title)
            return False
    else:
